...
class Calculation:
    ...
    @classmethod
    def singleton(cls, n):
        return cls("{}".format(n), n, is_singleton=True)
    ...
    def generate(cls, calca, calcb):
        ...
        for result, op in cls.operations(calca.result, calcb.result):
            expr1 = "{}".format(calca.expr) if calca.is_singleton else "({})".format(calca.expr)
            expr2 = "{}".format(calcb.expr) if calcb.is_singleton else "({})".format(calcb.expr)
            yield cls("{} {} {}".format(expr1, op, expr2), result)
...
def output(target, diff, results):
    print("The closest results differ from {} by {}. There are {} solutions:\n".format(target, diff, len(results)))
    for calculation in results:
        print("{} = {}".format(calculation.result, calculation.expr))
